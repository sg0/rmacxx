...
#ifdef USE_MPI_RMA
void fop(FILE * output, int max_iters, MPI_Comm comm, MPI_Win win)
#else
void fop(FILE * output, int max_iters, MPI_Comm comm, rmacxx::Window<char>& win)
#endif
{
...
#ifdef USE_MPI_RMA
#ifdef USE_MPI_FOP
        MPI_Fetch_and_op(&val, &result, MPI_CHAR, target, 0, MPI_REPLACE, win);
#else
        MPI_Get_accumulate(&val, 1, MPI_CHAR, &result, 1, MPI_CHAR, 
		target, 0, 1, MPI_CHAR, MPI_REPLACE, win); 
#endif
#else
        win(target, {0}, REPLACE, '0') >> result;
        //win(target, {0}, {0}, REPLACE, '0') >> &result;
#endif
    }
...
int main(int argc, char * argv[])
{
    ...
    fop(stdout, max_iters, MPI_COMM_WORLD, win);
