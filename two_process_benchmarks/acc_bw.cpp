#define BENCHMARK "MPI_Put%s Bandwidth Test"
...
/*Run PUT with flush */
void run_acc_with_flush (int rank)
{
    double t;
    int size, i, j;
#ifdef USE_MPI_RMA
...
#ifdef USE_MPI_RMA
		    MPI_Accumulate(sbuf+(j*size), size, MPI_CHAR, 1, disp + (j * size), size, MPI_CHAR, MPI_REPLACE, win);
#else
		    rwin(1, {j*size}, {j*size+size-1}, REPLACE) << sbuf+(j*size);
#endif
		}
#ifdef USE_MPI_RMA
		MPI_Win_flush(1, win);
