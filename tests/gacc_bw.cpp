#define BENCHMARK "MPI_Gacc%s Bandwidth Test"
...
char    sbuf_original[MYBUFSIZE], rbuf_original[MYBUFSIZE];
char    *sbuf=nullptr, *rbuf=nullptr;

void print_header (int); 
void print_bw (int, int, double);
void run_gacc_with_flush (int);
...
/*Run GACC with flush */
void run_gacc_with_flush (int rank)
{
    double t;
    int size, i, j;
#ifdef USE_MPI_RMA
    ...
    rbuf = (char *)align_buffer(&rbuf_original, page_size);
...
#ifdef USE_MPI_RMA
                    MPI_Get_accumulate(sbuf+(j*size), size, MPI_CHAR, rbuf+(j*size), 
                            size, MPI_CHAR, 1, disp + (j * size), size, MPI_CHAR, MPI_REPLACE, win);
#else
		    rwin(1, {j*size}, {j*size+size-1}, REPLACE, sbuf+(j*size)) >> rbuf+(j*size);
#endif
		}
#ifdef USE_MPI_RMA
		MPI_Win_flush(1, win);
