...
#ifdef USE_MPI_RMA
void get1(FILE * output, int max_iters, MPI_Comm comm, MPI_Win win)
#else
void get1(FILE * output, int max_iters, MPI_Comm comm, rmacxx::Window<int,LOCAL_VIEW,EXPR>& win)
#endif
{
    ...
    {
#ifdef USE_MPI_RMA
	    MPI_Put(&localval, 1, MPI_INT, target, 0, 1, MPI_INT, win);
#else
	    win(comm_rank, {0}) >> win(target, {0});
	    //win(comm_rank, {0}, {0}) >> win(target, {0}, {0});
#endif
    }
    ...
    // run for a single iteration iff profiling is turned on
#ifdef SDE_PROFILING
    for (int i = 0; i < 1; i++) 
#else
...
int main(int argc, char * argv[])
{
...
#else
    rmacxx::Window<int,LOCAL_VIEW,EXPR> win({1});
#endif
    ...
    get1(stdout, max_iters, MPI_COMM_WORLD, win);
