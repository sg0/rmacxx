...
#ifdef USE_MPI_RMA
void acc(FILE * output, int max_iters, MPI_Comm comm, MPI_Win win)
#else
void acc(FILE * output, int max_iters, MPI_Comm comm, rmacxx::Window<char>& win)
#endif
{
    ...
    {
#ifdef USE_MPI_RMA
	    MPI_Accumulate(&localval, 1, MPI_CHAR, target, 0, 1, MPI_CHAR, MPI_REPLACE, win);
#else
	    win(target, {0}, REPLACE) << localval;
	    //win(target, {0}, {0}, REPLACE) << &localval;
#endif
    }
...
int main(int argc, char * argv[])
{
    ...
    acc(stdout, max_iters, MPI_COMM_WORLD, win);
