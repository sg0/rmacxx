...
#ifdef USE_MPI_RMA
void get(FILE * output, int max_iters, MPI_Comm comm, MPI_Win win)
#else
void get(FILE * output, int max_iters, MPI_Comm comm, rmacxx::Window<char>& win)
#endif
{
    ...
    {
#ifdef USE_MPI_RMA
	    MPI_Get(&localval, 1, MPI_CHAR, target, 0, 1, MPI_CHAR, win);
#else
...
int main(int argc, char * argv[])
{
    ...
    get(stdout, max_iters, MPI_COMM_WORLD, win);
