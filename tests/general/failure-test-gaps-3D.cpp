...
int main(int argc, char *argv[])
{
    ...
    else if (rank == 3) { // process #4     if not commented out, should keep test from failing
        lo[0] = 3; lo[1] = 6; lo[2] = 0;
        hi[0] = 5; hi[1] = 9; hi[2] = 9;
    } 
    else { // process #3     creates a 3-wide gap in the x dimension
    ...
    std::vector<int> buff(240);
    std::fill(buff.begin(), buff.end(), 3);
    win({2,4,1},{7,8,8}) << buff.data();          //this should fail due to a gap in the location, instead failing at assertion
