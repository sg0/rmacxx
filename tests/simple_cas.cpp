...
#ifdef USE_MPI_RMA
void cas(FILE * output, int max_iters, MPI_Comm comm, MPI_Win win)
#else
void cas(FILE * output, int max_iters, MPI_Comm comm, rmacxx::Window<char>& win)
#endif
{
    ...
    {
#ifdef USE_MPI_RMA
        MPI_Compare_and_swap(&val, &cmp, &result, MPI_CHAR, target, 0, win);
#else
        win(target, {0}, '0', '0') >> result;
        //win(target, {0}, {0}, '0', '0') >> &result;
#endif
    }
...
int main(int argc, char * argv[])
{
    ...
    cas(stdout, max_iters, MPI_COMM_WORLD, win);
