...
#else
void put(FILE * output, int max_iters, MPI_Comm comm, rmacxx::Window<char>& win)
#endif
{
...
#else
    rmacxx::RMACXX_Subarray_t<char> foo( {0,0}, {1,1}, {1,1} );
#endif
    ...
    {
#ifdef USE_MPI_RMA
	    MPI_Put(&localval, 1, udtype, target, 0, 1, udtype, win);
#else
	    win(target, {0,0}, {0,0}) << foo( &localval );
#endif
    }
